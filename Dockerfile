...
# ── Build-time env vars ───────────────────────────────────────────────────────
# Public on-chain addresses and network config — baked in directly.
...
# Runtime-specific vars use placeholders — docker-entrypoint.sh replaces them
...
# Keeper reads these from env at runtime (supplied via --env-file .env.app)
ENV NEXT_PUBLIC_STREAM_PAY_ADDRESS=0x434ad66b34abe01c91eef1d24a1f2efede12c194
ENV SOMNIA_RPC_URL=https://dream-rpc.somnia.network
...
# Compiled keeper + its node_modules (viem, dotenv are already in standalone/node_modules)
